...
class SIFTdetect:
    ...
    def match_target_full(self, frame, bboxes):
            # not boxes availible
            if bboxes.shape[0] == 0:
                return None
            ...
            # best_crop = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            best_crop= frame[best_box[1]:best_box[3], best_box[0]:best_box[2]]
            kp, des = self.sift.detectAndCompute(best_crop, None)
...
print("Loading Model...")
session1 = ort.InferenceSession("./runs/detect/train5/weights/grey.onnx")
...
print("\n")
saved = 3
f = open("grey_fps.txt", 'w')
f2 = open("grey_fps_full.txt", 'w')
...
while True:
    ...
    # get camera image
    frame, in_img = get_frame_grey(cap)
    ...
    # apply sift
    target_idx = sifter.match_target_grey(frame, bboxes1)
    ...
    # Grey Mode B)
    start2 = time.time()
    sift_full = cv2.SIFT_create()
    matcher_full = cv2.BFMatcher()

    frame_full, in_img_full = get_frame_grey(cap)
    outputs2 = session2.run(None, {input_name1: in_img_full})[0] 
    outputs2, bboxes2 = process_output(outputs2, 0.7, 0.8, bbox=True)
    target_idx2 = sifter_full.match_target_full(frame_full, bboxes2)
    add_bboxes(frame_full, bboxes2, target_idx2)

    end2 = time.time()

    print(f"\033[A\r{1/(end1-start1):f} color fps, {1/(end2-start2):f} full sift fps, {outputs1.shape[0]} color detected, {outputs2.shape[0]} grey detected")
    ...
    cv2.imshow("ONNX FULL", frame_full)
